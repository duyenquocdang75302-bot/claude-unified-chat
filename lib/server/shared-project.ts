...
const SHARED_PROJECT_KEY = "claude-unified-chat:shared-project";
let memoryProject: ChatProject | null = null;
...
export function normalizeSharedProject(value: unknown): ChatProject | null {
  ...
  const project: ChatProject = {
    ...
    description: typeof input.description === "string" ? input.description.slice(0, 500) : "",
    instructions: typeof input.instructions === "string" ? input.instructions.slice(0, 30000) : "",
    defaultModel: typeof input.defaultModel === "string" && input.defaultModel.trim() ? input.defaultModel.trim().slice(0, 200) : "claude-sonnet-4-5",
    parameters: normalizeParameters(input.parameters),
    knowledge: Array.isArray(input.knowledge) ? input.knowledge.filter(isDocument).slice(0, 20) : [],
    createdAt: Number.isFinite(input.createdAt) ? Number(input.createdAt) : now,
    updatedAt: now,
...
export async function getSharedProject() {
  if (!hasPersistentKv()) return memoryProject;
  const payload = await kvCommand<string | null>(["GET", SHARED_PROJECT_KEY]);
  if (!payload) return null;
  try { return normalizeSharedProject(JSON.parse(payload)); } catch { return null; }
}
...
export async function saveSharedProject(value: unknown) {
  ...
  await kvCommand(["SET", SHARED_PROJECT_KEY, JSON.stringify(project)]);
  memoryProject = project;
  return project;
}
...
export async function deleteSharedProject() {
  ...
  await kvCommand(["DEL", SHARED_PROJECT_KEY]);
}
