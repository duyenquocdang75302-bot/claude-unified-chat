...
The application includes one server-side shared project. All authenticated accounts can use its instructions and knowledge files, while only an account with role `admin` can create, edit, upload, or delete it. Every account can also create and manage private projects stored only in that account's browser storage. On Vercel, shared project data and usage records are stored in Upstash Redis through `KV_REST_API_URL` and `KV_REST_API_TOKEN`; the local Cloudflare adapter remains available for legacy self-hosting. Chat histories and private projects remain separated per account in the browser. The project endpoints are `GET /api/project`, `PUT /api/project` (admin only), and `DELETE /api/project` (admin only). Reference image uploads support up to 20 images per message.
